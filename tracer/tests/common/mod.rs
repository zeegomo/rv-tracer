...
use rv_tracer::sim::{memory::Memory, LoadData, Tracer};
use std::fmt::{write, Debug};
use trace_defs::{TraceTable, MAIN_TRACE_WIDTH};
use winterfell::{
...
macro_rules! execute {
    ($ops:expr, $state:expr) => {{
        let load_data = LoadData {
            data: vec![(OP_ADDR, vec![$ops])],
        };
        // let mem = load_ops_at_addr(OP_ADDR, $ops);
    ...
    ($ops:expr, $state:expr, $pc:expr) => {{
        ...
        // let mem = load_ops_at_addr($pc, $ops);
...
fn load_ops_at_addr<O: Op>(addr: u32, op: &[O]) -> Memory {

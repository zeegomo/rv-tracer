...
impl Segment {
    pub fn filter_assertions_for_segment<E: FieldElement>(
        ...
        // the number for usable rows in a segment is one less than the segment length as one is used for padding
        // in addition, the first row of every segment after the first one overlaps with the second to last row of the previous segment
        let segment_start = self.segment_n * (segment_len - 2);
        assertions
            .iter()
            .filter(|assertion| {

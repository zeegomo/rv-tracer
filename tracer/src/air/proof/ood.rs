...
impl OodFrame {
    ...
    pub fn parse<E: FieldElement>(
        ...
        n_segments: usize,
    ) -> Result<Vec<ParsedOodFrame<E>>, DeserializationError> {
        let mut res = Vec::new();
        assert!(main_trace_width > 0, "trace width cannot be zero");
        assert!(num_evaluations > 0, "number of evaluations cannot be zero");

        let traces_len = self.trace_states.len() / n_segments;
        let evaluations_len = self.evaluations.len() / n_segments;

        for (traces, evaluations) in self
            .trace_states
            .chunks_exact(traces_len)
            .zip(self.evaluations.chunks_exact(evaluations_len))
        {
            let mut reader = SliceReader::new(traces);
            ...
            let mut reader = SliceReader::new(evaluations);
            let evaluations = E::read_batch_from(&mut reader, num_evaluations)?;
            if reader.has_more_bytes() {
                return Err(DeserializationError::UnconsumedBytes);
            }

            res.push((trace, evaluations));
        }

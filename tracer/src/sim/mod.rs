...
impl Tracer {
        ...
        E: FieldElement,
    {
        ...
        loop {
            ...
            log::trace!("about to execute {:?}", Op::parse(self.insn_at_pc()));
    ...
    pub fn build_trace(mut self) -> TraceTable<BaseElement> {
        let mut trace = Self::load_elf_to_memory(&self.data, &mut self.memory);
    ...
    fn load_elf_to_memory(elf: &LoadData, memory: &mut memory::Memory) -> Vec<Vec<BaseElement>> {
...
pub fn sim(elf: Elf32) -> TraceTable<BaseElement> {
    ...
    let tracer = Tracer::new(state, data);
    tracer.build_trace()
}

...
fn get_config(input: TokenStream) -> Result<Config, parse::SyntaxError> {
    let input = input.into_iter();
    require_keyword(&mut iter, "name")?;
    require_punct(&mut iter, "=")?;
    let name = require_ident(&mut iter)?;
    require_punct(&mut iter, ",")?;

    require_keyword(&mut iter, "opcode")?;
    require_punct(&mut iter, "=")?;
    let opcode = require_value(&mut iter)?;
    require_punct(&mut iter, ",")?;
    let ops = input
        .into_iter()
        .flat_map(|t| match t {
...
#[proc_macro]
pub fn air(_item: TokenStream) -> TokenStream {
    let name = todo!();
